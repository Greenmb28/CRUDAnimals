...
const login = () => {
  ...
  // Add a click hander for the cats button
  $("#cat").on("click", createPanel);
};
...
const updateHandler = (index) => {
  ...
  $(".save_update").on("click", function () {
    ...
    displayLine.html(`
      <div class="info">${animals[index].animal_name} the ${animals[index].breed} <img src="./images/genericdog.png"/></div>
...
const createHandler = () => {
  ...
  $(".save_new").on("click", function () {
    ...
    let animalLine = $(
      `<div class="displayLine">
          <div class = "info">${newAnimal.animal_name} the ${newAnimal.breed} <img src="./images/genericdog.png"/> </div>
          <div class = "crud_buttons"><button class="view">View</button><button class="update">Update</button><button class="remove">Remove</button></div>
          <div class="showInfo"></div>
       </div>`
    );
...
///So what i did to fix the images was i changed the img sorce on line 174 from the picture to match the name
//to the pic of the generic dog image. I alos changed the image on line 258 from jpeg to png in my files. 
//The way that i was able to put my files into folders and connect them was i added
//../js/ to the src in the html and it fixed everything 